...
(function ($) {
    ...
    $.Promise.prototype._invokeError = function (ex, delay) {
        if (delay > 0) {
            ...
            this._apply(function () {
                callback({message: ex.message, error: ex.complete, origin: ex.origin, code: ex.code});
            });
        }
    };
	...
	*/
	...
	$.redirectToInstallPage = function () {
		document.location.href = $._installUrl + (this.brand || '') + '?returnUrl=' + encodeURIComponent(document.URL) + '&jslib=2.7.0';
	};
			...
			$._requestHandler = new function () {
						...
						for (var requestId in pendingRequests) {
							...
							if (pendingRequest.sendFailed) {
								...
								if (result !== null) {
								    ...
								    pendingRequest.promise._invokeError({ message: 'The operation has timed out', error: 'The operation has timed out', origin: 'helper', code: 'addon_timeout'});
									removePendingRequest = true;
								}
							}
				...
				var sendCommand = function (context, command, request, responseProcessor) {
					if (getAddon()) {
						...
						if (sendCommandError) {
						    context.promise._invokeError({message: 'Failed to send command to add-on', error: sendCommandError, origin: 'helper', code: 'addon_send_command_failure'}, 200);
							pendingRequests[requestId].sendFailed = true;
						}
					} else {
					    context.promise._invokeError({message: 'Add-on not detected', error: 'Add-on not detected', origin: 'helper', code: 'addon_not_detected'}, 200);
					}
				};
